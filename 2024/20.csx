...
var input = File.ReadAllText("2024/inputs/20.real.txt");
...
skips1.Count(i => i >= 100).Dump("Part 1").Should().BeOneOf(0, 1393);
...
foreach (var (i, pos) in steps.Take(steps.Count - 3).Index())
{
	for (var x = -21; x <= 21; x++)
		for (var y = -21; y <= 21; y++)
		{
			...
			var newPos = pos + (x, y);
			if (!map.IsInBounds(newPos) || map[newPos] is 'S' or '#')
				continue;
...
skips2.Select(s => s.skips).Count(i => i >= 100).Dump("Part 2").Should().BeOneOf(0, 990096);
