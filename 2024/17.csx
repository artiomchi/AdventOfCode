...
var input = File.ReadAllText("2024/inputs/17.real.txt");
var match = Regex.Match(input, @"A: (?<a>\d+)\s+\w+ B: (?<b>\d+)\s+\w+ C: (?<c>\d+)\s+Program: (?<i>[\d,]+)");
var ra = Convert.ToInt32(match.Groups["a"].Value);
var rb = Convert.ToInt32(match.Groups["b"].Value);
var rc = Convert.ToInt32(match.Groups["c"].Value);
...
string.Join(",", RunProgram(ra, inputs)).Dump("Part 1").Should().BeOneOf("4,6,3,5,6,3,5,2,1,0", "2,1,4,7,6,0,3,1,4");
...
//var a = 280214269591552;
//while (a < 280214403809279)
//{
//	string.Join(',', RunProgram(a, inputs)).Dump(a.ToString());
//	a += 6710886;
//}

// Enumerable.Range(-500, 1000)
// 	.Select(i => (i, r: RunProgram(266932601404463 + i, inputs)))
// 	.Where(x => x.r.SequenceEqual(inputs))
// 	.Select(x => 266932601404463 + x.i)
// 	.Order()
// 	.First()
// 	.Dump("Part 2")
// 	.Should().Be(266932601404433);
// return;
...
for (int i = 1; i <= inputs.Length; i++)
{
	...
	// (
	// 	$"{min} - {string.Join(',', RunProgram(min, inputs))}",
	// 	$"{max} - {string.Join(',', RunProgram(max, inputs))}"
	// ).Dump(i.ToString());
}

min.Dump("Part 2").Should().Be(266932601404433);


bool Test(long ra, int[] inputs, int digits)
{
	var match = inputs.TakeLast(digits);
	var results = Enumerable.Range(-2, 5)
		.Select(i =>
		{
			try
			{
				return RunProgram(ra + i, inputs);
			}
			catch
			{
				return [];
		...
		.Select(r => r.Count == 0 
			? 0 
			: r.Count != inputs.Length
			? -2
			: r.TakeLast(digits).SequenceEqual(match) 
			? 1 
			: -1)
		.ToArray();
	if (results[2] != 1)
	...
	return true;
	return (results[0] >= 0 && results[1] >= 0) || (results[3] >= 0 && results[4] >= 0);
}
